...
from legacyutils.ADB import EnrollmentNamespace as e

MODE='NOBLE'
#MODE='COMER'

# Instance specific Ids for tricky colleges
if MODE=='NOBLE':
    ARRUPE = '001E000001CiAmAIAV'
    LOYOLA = '001E000000KMhd5IAD'
    UICHonors = '001E000001CiAtTIAV'
    UIC = '001E000000KMhckIAD'
elif MODE=='COMER':
    ARRUPE = '00146000004iTjpAAE'
    LOYOLA = '00146000004iTjtAAE'
    UICHonors = '00146000004iTkuAAE'
    UIC = '00146000004iTl4AAE'
...
def colleges_match(dbc, nscc):
    """Mostly a straight compare, but adds Arrupe/Loyola equivalence"""
    return dbc == nscc or (
            dbc == ARRUPE and
            nscc == LOYOLA) or (
            dbc == UICHonors and
...
class PerfectMatchButArrupe(MatchCase):
    def comp(self, db, nsc):
        return True if ( db[0] == nsc[0] and # Student
                         colleges_match(db[1], nsc[1]) and #Arrupe/Loyola or UICH
                         db[2] == nsc[2] and # Start Date
                         db[3] == nsc[3] and # End Date
                         db[5] == nsc[5]     # Status
                       ) else False
...
class PerfectMatchFuzzyStartButArrupe(MatchCase):
    def comp(self, db, nsc):
        if not db[2]: #makes sure the database has a non-blank start date
            return False
        return True if ( db[0] == nsc[0] and # Student
                         colleges_match(db[1], nsc[1]) and #Arrupe/Loyola or UIC Honors
                         db[3] == nsc[3] and # End Date
                         db[5] == nsc[5] and # Status
                         fuzzy_start_match(db[2], nsc[2]) # Start Date
                       ) else False
...
class SCSt_GG_Match(MatchCase):
    def comp(self, db, nsc):
        if not db[2]: #makes sure the database has a non-blank start date
            return False
        return True if ( db[0] == nsc[0] and # Student
                         colleges_match(db[1], nsc[1]) and # College
...
class SCMatch(MatchCase): # Catch-all case that must come last
    def comp(self, db, nsc):
        return True if ( db[0] == nsc[0] and # Student
                         colleges_match(db[1], nsc[1])     # College
                       ) else False
...
def build_match_cases():
    ...
    return [
            ...
            PerfectMatchButArrupe('Perfect Match but Arrupe'),
            PerfectMatchFuzzyStart('Perfect Match w Start +/- 60 days off'),
            PerfectMatchFuzzyStartButArrupe('Perfect Match w Start +/- 60 days off but Arrupe'),
