...
A summary of modules:
    ...
    push_attachment(sf, object_id, filename) --> response
    Pushes an attachment specified by the filename using the REST API
'''
import base64
import requests
import json
...
import salesforce_secrets as s_s
import sys # for stderr

def getSF():
    try:
        # from ... import salesforce_secrets as s_s
        un = s_s.SF_LIVE_USERNAME
        pw = s_s.SF_LIVE_PASSWORD
        st = s_s.SF_LIVE_TOKEN
    except:
...
def push_attachment(sf, object_id, filename):
    '''
    Pushes an attachment (filename) to the object
    '''
    url = sf.base_url + 'sobjects/Attachment/'
