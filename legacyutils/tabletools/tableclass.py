...
class Table():
    def __init__(self, source, encoding=None):
        ...
        if type(source) is list:
            ...
            try:
                fullList = tt.grab_csv_table(source, encoding=encoding)
            except:
                raise Exception(errmsg)
        else:
            raise Exception(errmsg)
    ...
    def __repr__(self):
        basis = self.get_full_table()
        base_str = 'Table with %d columns and %d data rows. Sample:\n' % (
                   len(basis[0]), len(basis)-1)
        d = len(basis)
        if d > 3:
            base_str = base_str + '\n%14s: %-18s %-18s ... %-18s\n' % (
                        'Header', 'Row 1', 'Row 2', 'Row '+str(len(basis)-1))
        else:
            base_str = base_str + '\n%14s: %-18s %-18s\n' % (
                        'Header', 'Row 1', 'Row 2')

        for i in range(len(basis[0])):
            new_str = '%14s: %-18s %-18s ... %-18s\n' % (
                       str(basis[0][i]),
                       str(basis[1][i]) if d > 1 else '',
                       str(basis[2][i]) if d > 2 else '',
                       str(basis[-1][i] if d > 3 else '',))
    ...
    def get_match_rows_table(self, column, match_value):
        '''Returns a full Table of the rows that match a given value
        in the specified column'''
        new_data = list(self.get_match_rows(column, match_value))
        tt.add_header(new_data, self.header)
