...
def reduce_table_by_checkbox(raw_data, column_name, prompt,
                            default=True, type_check=str):
    '''Lets user specify which rows to retain in a list of lists table
    by letting them select checkboxes of the unique values in the 
    specified column name'''
    hc = tt.slice_header(raw_data) # will need to add back later
    options = list(set([x[hc[column_name]] for x in raw_data]))
    options = [h for h in options if type(h) is type_check]
    options.sort()
    if default=='Single':
        picked = [get_buttons_answer(options, prompt)]
    else:
        pick_default = 1 if default else 0 # switch from logical to 1/0
        picked = check_pick_from_list(options, prompt, pick_default)
    reduced_table = [x for x in raw_data if x[hc[column_name]] in picked]
    tt.add_header(reduced_table, hc)
