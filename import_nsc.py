...
'''
from legacyutils.tkintertools import tktools
from legacyutils.ADB import AlumniDatabaseInterface as aDBi
from legacyutils.tabletools import tableclass as tc
        ...
        for i in range(1,len(s_c_table)):
            ...
            if (days_diff > daysgap or #too many days apart
               (    s_c_table[i-1][hd['Grad Date']] and #continuing after
                ...
                # Probably a new enrollment starting with this one
                # but it's possible this is a single date graduation enrollment
                if (s_c_table[i][hd['Start Date']] == s_c_table[i][hd['End Date']] and
                    s_c_table[i][hd['End Date']] == s_c_table[i][hd['Grad Date']]):
                    # This is an outlier grad date row that belongs above (same enrollment)
                    new_enr.append(i)
                else:  # Actually a new enrollment
...
def finalize_output_columns(combined_nsc, fix_attending=True):
...
def main(nsc, sch, deg, out, daysgap, effdate, fix_attending):
    ...
    final_output_table = finalize_output_columns(combined_nsc, fix_attending)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-sch', dest='sch', action='store', help=sch_help,
            default='inputs/collegelist.csv')
    deg_help='Degrees table (provided with this file)'
    parser.add_argument('-deg', dest='deg', action='store', help=deg_help,
            default='inputs/degreelist.csv')
    out_help='Output filename (csv)'
    parser.add_argument('-end_attending', dest='fix_attending',
            action='store_false', help='Keep end date for attending enr.')
    ...
    if args.effdate is None:
        #args.effdate='08/15/2019'
        args.effdate='11/18/2019'
    main(   arg_dict['nsc'][0],
            arg_dict['sch'][0],
            arg_dict['deg'][0],
            arg_dict['out'][0],
